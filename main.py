    ...
    with open(fname) as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                geoaq = GeoAnalyticalQuestion(row[0], row[1], row[3], row[4], row[5], 'geoanalytical')
                res.append(geoaq)
                line_count += 1
    ...
    with open(fname, encoding="utf8") as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                # print(f'id: \t{row[0]}, source: {row[1]}, year: {row[3]}, title: {row[4]} and question: {row[5]}.')
                geoaq = GeoAnalyticalQuestion(row[2], '2018', 'MSMARCO',
                                              'The other dataset', row[0], 'search queries')
                res.append(geoaq)
                line_count += 1
        print(f'Processed {line_count} lines.')
        return res
...
# geoaqs = read_msmarco_dataset('data/geoaq/MSMARCO.csv')
questions_incorrect = []
results = []
count = 0
for geoaq in geoaqs:
    try:
        ...
        count += 1
        logging.info('now processing the record number ::: {}'.format(count))
        # if count == 500:
        # break
    except:
